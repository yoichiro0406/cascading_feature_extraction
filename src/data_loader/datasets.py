...
class collate_fn:
    def __init__(
        ...
        self.random_dense = random_dense
        self.uneven = uneven
    ...
    def __call__(self, batch):
        ...
        if self.uneven:
            for b in batch:
                self.max_points = min(self.max_points, b["points_src"].shape[0])
                self.max_points = min(self.max_points, b["points_ref"].shape[0])

        for key in elem:

            if key == "points_src" or key == "points_ref":
                if self.random_dense:
                    log_min, log_max = np.log2(self.min_points), np.log2(
                        self.max_points
                    )
                    log_num_points = np.random.uniform(log_min, log_max)
                    num_points = np.power(2, log_num_points)
                    num_points = int(num_points * self.proportion)

                else:
                    num_points = int(self.max_points)

            for b in batch:
                l = len(b[key])
                new_idx = np.random.choice(range(l), replace=False, size=num_points)
                b[key] = b[key][new_idx]

        batch = {
            key: torch.stack([torch.as_tensor(b[key]) for b in batch]) for key in elem
        }
        return batch
...
def get_train_datasets(args: argparse.Namespace):
    ...
    _logger.info(
        "Train transforms: {}".format(
            ", ".join([type(t).__name__ for t in train_transforms])
        )
    )
    _logger.info(
        "Val transforms: {}".format(
            ", ".join([type(t).__name__ for t in val_transforms])
        )
    )
    ...
    if args.dataset_type == "modelnet_hdf":
        train_data = ModelNetHdf(
            args.dataset_path,
            subset="train",
            categories=train_categories,
            transform=train_transforms,
        )
        val_data = ModelNetHdf(
            args.dataset_path,
            subset="test",
...
def get_test_datasets(args: argparse.Namespace):
    ...
    _logger.info(
        "Test transforms: {}".format(
            ", ".join([type(t).__name__ for t in test_transforms])
        )
    )
    test_transforms = torchvision.transforms.Compose(test_transforms)

    if args.dataset_type == "modelnet_hdf":
        test_data = ModelNetHdf(
            args.dataset_path,
            subset="test",
...
def get_transforms(
    ...
    if noise_type == "clean":
    ...
    elif noise_type == "crop_scale":
        train_transforms = [
            Transforms.RandomScale(scale_min, scale_max),
            Transforms.SplitSourceRef(),
            Transforms.RandomCrop(partial_p_keep),
            Transforms.RandomTransformSE3_euler(rot_mag=rot_mag, trans_mag=trans_mag),
            Transforms.Resampler(num_points),
        ...
        test_transforms = [
            Transforms.SetDeterministic(),
            Transforms.RandomScale(scale_test, scale_test),
            Transforms.SplitSourceRef(),
            Transforms.RandomCrop(partial_p_keep),
            Transforms.RandomTransformSE3_euler(rot_mag=rot_mag, trans_mag=trans_mag),
            Transforms.Resampler(num_points),
    ...
    elif noise_type == "crop_scale_dense":
        train_transforms = [
            Transforms.RandomScale(scale_min, scale_max),
            Transforms.SplitSourceRef(),
            Transforms.RandomCrop(partial_p_keep),
    ...
    elif noise_type == "kitti":
        train_transforms = [
            Transforms.RandomTransformSE3_euler(rot_mag=rot_mag, trans_mag=trans_mag),
            # Transforms.RandomTransformSE3_euler_kitti(),
            # Transforms.ToInit(),
            Transforms.Resampler(num_points),
            Transforms.ShufflePoints(),
        ]
...
class ModelNetHdf(Dataset):
    def __init__(
        ...
        with open(os.path.join(dataset_path, "{}_files.txt".format(subset))) as fid:
            ...
            h5_filelist = [
                x.replace("data/modelnet40_ply_hdf5_2048/", "") for x in h5_filelist
            ]

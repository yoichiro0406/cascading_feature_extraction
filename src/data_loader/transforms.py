...
class RandomJitter:
    ...
    def __init__(self, gaussian_scale=0.01, gaussian_clip=0.05):
        self.gaussian_scale = gaussian_scale
        self.gaussian_clip = gaussian_clip

    def jitter(self, pts, scale):

        gaussian_scale = self.gaussian_scale * scale
        gaussian_clip = self.gaussian_clip * scale

        noise = np.clip(
            np.random.normal(0.0, scale=gaussian_scale, size=(pts.shape[0], 3)),
            a_min=-gaussian_clip,
            a_max=gaussian_clip,
        )
    ...
    def __call__(self, sample):
        ...
        if "points" in sample:
            sample["points"] = self.jitter(sample["points"], sample["scale"])
        else:
            sample["points_src"] = self.jitter(sample["points_src"], sample["scale"])
            sample["points_ref"] = self.jitter(sample["points_ref"], sample["scale"])
...
class RandomScale:
    def __init__(self, scale_min=0.0, scale_max=0.0):
        self.scale_min = scale_min
        self.scale_max = scale_max

    def scale(self, pts):
        scale = np.random.uniform(self.scale_min, self.scale_max)
        # scale = np.random.normal((self.scale_max + self.scale_min) / 2.0, (self.scale_max - self.scale_min) / 2.0)
        scale = np.power(2, scale)
        scaled_pts = pts[:, :3] * scale
        normals = pts[:, 3:6]
        scaled_pts = np.concatenate((scaled_pts, normals), axis=-1)
        return scaled_pts, scale

    def __call__(self, sample):
        if "points" in sample:
            sample["points_raw"] = sample.pop("points").copy()
            # sample['points_raw'], scale = self.scale(sample.pop('points'))
            sample["points_raw"], scale = self.scale(sample["points_raw"])
            sample["scale"] = scale
        else:
...
class ToInit:  # TODO 直す
    ...
    def transform(self, tensor, trans_init):
        return self.apply_transform(tensor, trans_init)
    ...
    def __call__(self, sample):
        ...
        if "points" in sample:
            sample["points"], _, _ = self.transform(
                sample["points"], sample["init_trans"]
            )
        else:
            src_transformed, transform_r_s, transform_s_r = self.transform(
                sample["points_src"], sample["init_trans"]
            )
            sample["transform_gt"] = transform_r_s  # Apply to source to get reference
            sample["points_src"] = src_transformed
...
class RandomTransformSE3_euler_kitti(RandomTransformSE3):  # TODO まだ
    """Same as RandomTransformSE3, but rotates using euler angle rotations
    ...
    def generate_transform(self):
        ...
        # Generate rotation
        anglex = np.random.uniform() * np.pi * self._rot_mag_x / 180.0
        angley = np.random.uniform() * np.pi * self._rot_mag_y / 180.0
        anglez = np.random.uniform() * np.pi * self._rot_mag_z / 180.0

"""Evaluate CFENet. Also contains functionality to compute evaluation metrics given transforms
...
from arguments import cfenet_eval_arguments
from common.math import se3
from common.math.so3 import dcm2euler
from common.math_torch import se3
from common.misc import prepare_logger
from common.pyplot import save_plot
...
def compute_metrics(data: Dict, pred_transforms) -> Dict:
    ...
    with torch.no_grad():
        ...
        r_gt_euler_deg = dcm2euler(
            gt_transforms[:, :3, :3].detach().cpu().numpy(), seq="xyz"
        )
        r_pred_euler_deg = dcm2euler(
            pred_transforms[:, :3, :3].detach().cpu().numpy(), seq="xyz"
        )
        ...
        if "points_raw" in data:
            ...
            src_transformed = se3.transform(pred_transforms, points_src)
            src_gt = se3.transform(gt_transforms, points_src)
            dist_src = torch.min(square_distance(src_transformed, src_gt), dim=-1)[0]
            chamfer_dist = torch.mean(dist_src, dim=1)
...
def inference(data_loader, model: torch.nn.Module):
    ...
    with torch.no_grad():
        for (i, val_data) in track(
            enumerate(data_loader), description="", total=len(data_loader)
        ):

            rot_trace = (
                val_data["transform_gt"][:, 0, 0]
                + val_data["transform_gt"][:, 1, 1]
...
def evaluate(pred_transforms, data_loader: torch.utils.data.dataloader.DataLoader):
    ...
    for data in track(data_loader, description=""):
    ...
    for i_iter in range(len(metrics_for_iter)):
        ...
        print_metrics(
            _logger, summary_metrics, title="Evaluation result (iter {})".format(i_iter)
        )
...
def get_model():
    ...
    if _args.resume is None:
        print("Evaluation without pretrained weights !")

    model = getattr(models, _args.method).get_model(_args)
    ...
    if torch.cuda.is_available():
        ...
        saver.load(_args.resume, model, map_cpu=True)
...
def main():
    ...
    test_loader = torch.utils.data.DataLoader(
        test_dataset, batch_size=_args.val_batch_size, shuffle=False
    )

    if _args.transform_file is not None:
        _logger.info(
            "Loading from precomputed transforms: {}".format(_args.transform_file)
        )
        _logger.info(
            "scale min: {}  scale max: {}".format(_args.scale_min, _args.scale_max)
        )
        pred_transforms = np.load(_args.transform_file)
        endpoints = {}
    else:
        model = get_model()
        pred_transforms, endpoints = inference(
            test_loader, model
    ...
    save_eval_data(
        pred_transforms, endpoints, eval_metrics, summary_metrics, _args.eval_save_path
    )

...
Modified from:
    1.  Pytorch Implementation of PointNet and PointNet++
        https://github.com/yanx27/Pointnet_Pointnet2_pytorch
    2.  RPM-Net: Robust Point Matching using Learned Features
...
def square_distance(src, dst):
    ...
    dist += torch.sum(src * src, dim=-1)[:, :, None]
    dist += torch.sum(dst * dst, dim=-1)[:, None, :]
    return dist
...
class faiss_batch:
    ...
    def __call__(self, x, k):
        batch_dists = torch.empty((x.shape[0], x.shape[1], k))
        batch_group_idx = torch.empty((x.shape[0], x.shape[1], k), dtype=torch.long)

        for i in range(x.shape[0]):
            self.lock.acquire()
            self.index.reset()
            self.index.add(x[i].contiguous())
            dists, group_idx = self.index.search(x[i].contiguous(), k=k + 1)
            self.lock.release()
            batch_dists[i], batch_group_idx[i] = dists[..., 1:], group_idx[..., 1:]

        return batch_dists, batch_group_idx


try:
    topk_faiss = faiss_batch()
except:
    pass


def topk(new_xyz, xyz, nsample):
    sqrdists = square_distance(new_xyz, xyz)
    dists, group_idx = sqrdists.topk(nsample + 1, largest=False, sorted=False)
    return dists[..., 1:], group_idx[..., 1:]


def all_sort(radius, new_xyz, xyz, itself_indices, nsample):
    device = xyz.device
    B, N, C = xyz.shape
    _, S, _ = new_xyz.shape
    ...
    if itself_indices is not None:
        ...
        row_indices = torch.arange(S, dtype=torch.long, device=device)[None, :].repeat(
            B, 1
        )  # (B, S)
    ...
    if itself_indices is not None:
        ...
        group_first = group_idx[:, :, 0].view(B, -1, 1).repeat([1, 1, nsample])
...
def exclude_points(dists, group_idx, radius):
    B, _, _ = dists.shape

    out_of_r_idx = dists > radius ** 2

    idx_first = group_idx[..., 0]
    idx_first = out_of_r_idx * idx_first.view(B, -1, 1)
    group_idx = group_idx * ~out_of_r_idx + idx_first

    return group_idx


def query_ball_point(radius, nsample, xyz, new_xyz, itself_indices=None, knn=None):

    if knn == "all_sort":
        group_idx = all_sort(radius, new_xyz, xyz, itself_indices, nsample)
    elif "faiss" in knn:
        dists, group_idx = topk_faiss(xyz, nsample)
    else:
        dists, group_idx = topk(new_xyz, xyz, nsample)

    if "select" in knn:
        group_idx = exclude_points(dists, group_idx, radius)
    else:
        pass
...
def sample_and_group_multi(
    ...
    if npoint > 0:
        ...
        S = xyz.shape[1]
        fps_idx = torch.arange(0, xyz.shape[1], device=xyz.device)[None, ...].repeat(
            xyz.shape[0], 1
        )
        new_xyz = xyz
        nr = normals[:, :, None, :]
    ...
    idx = query_ball_point(
        radius, nsample, xyz, new_xyz, fps_idx, knn=knn
    )  # (B, npoint, nsample)
    grouped_xyz = index_points(xyz, idx)  # (B, npoint, nsample, C)
    ...
    if ppf_type == "angle":  # RPMNet original
        nr_d = angle(nr, d)
        ni_d = angle(ni, d)
        nr_ni = angle(nr, ni)
    elif ppf_type == "inner_prod":  # proposed
        nr_d = torch.sum(nr * d, dim=-1) / (d_norm + _EPS)
        ni_d = torch.sum(ni * d, dim=-1) / (d_norm + _EPS)
        nr_ni = torch.sum(nr * ni, dim=-1)
    else:
        raise NotImplementedError

    xyz_feat = d  # (B, npoint, n_sample, 3)
    ppf_feat = torch.stack(
        [nr_d, ni_d, nr_ni, d_norm], dim=-1
    )  # (B, npoint, n_sample, 4)
